...
class MainWindow(QMainWindow):
    ...
    def update_clients_list(self):
        ...
        try:
            ...
            self.clients_table.setItem(row, 0, QTableWidgetItem("💻 本机"))
            self.clients_table.setItem(row, 1, QTableWidgetItem(self.controller.easytier.virtual_ip or "unknown"))
            self.clients_table.setItem(row, 2, QTableWidgetItem("-"))
            
            # 添加其他设备
            for peer in peers:
                if peer.get('conn_type') != 'Local':  # 跳过本机
                    row += 1
                    self.clients_table.insertRow(row)
                    
                    hostname = peer.get('hostname', 'Unknown')
                    ipv4 = peer.get('ipv4', 'Unknown')
                    latency = peer.get('latency', '-')
                    
                    self.clients_table.setItem(row, 0, QTableWidgetItem(f"🖥️ {hostname}"))
                    self.clients_table.setItem(row, 1, QTableWidgetItem(ipv4.split('/')[0] if '/' in ipv4 else ipv4))
                    self.clients_table.setItem(row, 2, QTableWidgetItem(latency))
            
            logger.info(f"更新客户端列表: 总计 {row + 1} 台设备")
